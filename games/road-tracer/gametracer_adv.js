...
backgroundGameImg.src = "/assets/images/roadbackg3.png";

const roadImg = new Image();
roadImg.src = "/assets/images/road3.png";

const roadMaskImg = new Image();
roadMaskImg.src = "/assets/images/road3mask.png";
...
function initGame() {
    ...
    const roadHeight = canvas.height * 0.75;
    ...
    const carSize = 80;
    state.carInitialX = 180;
    state.carInitialY = canvas.height / 2 + 150;
    ...
    // Initialize 2 road cones at center
    const coneSize = 70;
    state.roadCones = [
        {
...
function getPixelColor(x, y) {
    ...
    // Calculate road dimensions maintaining aspect ratio
...
function checkCarConeCollision() {
    if (!state.car || !state.roadCones) return;

    for (let cone of state.roadCones) {
        const dx = state.car.x - (cone.x + cone.width / 2);
        const dy = state.car.y - (cone.y + cone.height / 2);
        const distance = Math.sqrt(dx * dx + dy * dy);
        
        // Check collision (sum of radii)
        const collisionDistance = (state.car.width / 2) + (cone.width / 2);
        
        if (distance < collisionDistance && !cone.hit) {
            console.log("Car hit road cone!");
            state.roadConeHits++;
            cone.hit = true; // Mark cone as hit to prevent multiple counts
            ...
            setTimeout(() => {
                resetCarToStart();
                // Reset cone hit state after car returns to start
                cone.hit = false;
            }, 300);
            break;
        }
...
function updateCar() {
    ...
    // Check car collision with road cones
    checkCarConeCollision();
}
...
function drawScene() {
    ...
    // Draw 2 road cones at center
    if (roadConeImg.complete && state.roadCones) {
        state.roadCones.forEach(cone => {
            ctx.drawImage(roadConeImg, cone.x, cone.y, cone.width, cone.height);
        });
    }
    ...
    if (policeImg.complete) {
        ctx.drawImage(policeImg, 20, canvas.height / 2 + 160, iconSize, iconSize);
    }
    
    // Left side: arrow (reduced by 20%, rotated -90 degrees)
    const go_arrowSize = iconSize * 0.8;
    if (goArrowImg.complete) {
        ctx.save();
        ctx.translate(30 + go_arrowSize / 2, canvas.height / 2 + 90 + go_arrowSize / 2);
        ctx.rotate(-Math.PI / 2);
        ctx.drawImage(goArrowImg, -go_arrowSize / 2, -go_arrowSize / 2, go_arrowSize, go_arrowSize);
        ctx.restore();
    }
    ...
    if (stopImg.complete) {
        ctx.drawImage(stopImg, canvas.width / 2 + 150, canvas.height / 2 - 140, stopSize, stopSize);
    }
...
function calculatePinchAccuracy() {
    // pinch accuracy (%) = (Success pinch until finish line / (total pinch attempts - (number hit boundary + number hit road cone))) * 100
    const totalObstacleHits = state.boundryHits + state.roadConeHits;
    const validAttempts = state.attempts - totalObstacleHits;
...
function calculateTraceStability() {
    ...
    // boundryhitrate = (the number car hit boundary + number car hit road cone) / total time taken
    const finalTime = state.finalElapsed ?? 1;
    const totalObstacleHits = state.boundryHits + state.roadConeHits;
    const boundaryHitRate = totalObstacleHits / finalTime;
    ...
    return Math.max(0, Math.round(stability * 100) / 100);
}
...
async function saveGameResult() {
    ...
    const roadConeHits = state.roadConeHits;
    ...
    if (insertError) {
        ...
        console.log(`Road cone hits: ${roadConeHits}`);
