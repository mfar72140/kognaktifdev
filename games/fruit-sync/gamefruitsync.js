...
const TARGET_SCORE = 3; // Win condition (3 successful matches)

// Card positions
const CARD_WIDTH = 165; // Increased by 10%
const CARD_HEIGHT = 220; // Increased by 10%
const CARD_SPACING = 60; // Increased by 20%
...
function preloadAssets() {
    const promises = [];
    const images = [
        coverImg, backgroundImg, cardAImg, cardBImg,
...
function initializeCards() {
    const totalWidth = (CARD_WIDTH * 3) + (CARD_SPACING * 2);
    const startX = (canvas.width - totalWidth) / 2;
    const centerY = canvas.height / 2;

    // Randomize which card is different
    const differentCardIndex = Math.floor(Math.random() * 3);
    ...
    state.cards = [
        ...
        },
        {
            ...
            width: CARD_WIDTH,
            height: CARD_HEIGHT,
            type: differentCardIndex === 1 ? 'B' : 'A',
            pulseScale: 1,
            matched: false,
            floatOffset: Math.PI * 0.66,
            floatSpeed: 1 + Math.random() * 0.5,
            glowIntensity: 0
        },
        {
            ...
            width: CARD_WIDTH,
            height: CARD_HEIGHT,
            type: differentCardIndex === 2 ? 'B' : 'A',
            pulseScale: 1,
            matched: false,
            floatOffset: Math.PI * 1.33,
            floatSpeed: 1 + Math.random() * 0.5,
            glowIntensity: 0
        }
    ];
...
function drawScene() {
    ...
    state.cards.forEach(card => {
        ...
        const cardImg = card.type === 'A' ? cardAImg : cardBImg;
...
function fadeInWinOverlay() {
    ...
    function drawOverlay(now) {
        ...
        ctx.fillText("You matched all the cards!", canvas.width / 2, cardY + 250);
