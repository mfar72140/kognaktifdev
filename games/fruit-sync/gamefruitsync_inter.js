...
const MATCHES_PER_ROUND = 2; // 2 successful matches = 1 round

// Card positions
const CARD_WIDTH = 132; // Reduced by 20%
const CARD_HEIGHT = 176; // Reduced by 20%
const CARD_SPACING = 48; // Reduced by 20%
...
function preloadAssets() {
    const promises = [];
    const images = [
        coverImg, backgroundImg, cardAImg, cardIImg, cardBImg,
...
function initializeCards() {
    ...
    // Uniform card positions: 2 top, 1 center, 2 bottom
    const padding = 60;
    ...
    // Top row: 2 cards
    const topY = padding;
    const topLeftX = centerX - CARD_WIDTH - CARD_SPACING / 2 - 100;
    const topRightX = centerX + CARD_SPACING / 2 + 100;

    // Middle row: 1 card (centered)
    const middleY = centerY - CARD_HEIGHT / 2;
    const middleX = centerX - CARD_WIDTH / 2;

    // Bottom row: 2 cards
    const bottomY = canvas.height - padding - CARD_HEIGHT;
    const bottomLeftX = centerX - CARD_WIDTH - CARD_SPACING / 2 - 100;
    const bottomRightX = centerX + CARD_SPACING / 2 + 100;
    ...
    if (state.leftSelectedCard && state.rightSelectedCard) {
        if (state.leftSelectedCard.type === state.rightSelectedCard.type &&
            ...
            setTimeout(() => {
                ...
                // Check if round is complete (2 matches = 1 round)
...
async function updateHandDetection() {
    ...
    try {
        ...
        if (results.landmarks && results.landmarks.length > 0) {
            ...
            // Check for match when both hands grasping
...
function drawScene() {
    ...
    state.cards.forEach(card => {
        ...
        const cardImg = card.type === 'A' ? cardAImg : (card.type === 'I' ? cardIImg : cardBImg);
...
function calculateGraspPrecision() {
    ...
    const graspPrecision = (12 / graspRatio) * 100;
