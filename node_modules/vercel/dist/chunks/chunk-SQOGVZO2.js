...
function formatCsv(data, groupBy, rollupColumn) {
...
function formatSchemaListCsv(events) {
...
function formatSchemaDetailCsv(event) {
  ...
  const measureRows = event.measures.map(
    (m) => `${escapeCsvValue(m.name)},${escapeCsvValue(m.label)},${escapeCsvValue(m.unit)}`
  );
