...
/* =========================
    ======== Assets =========
    ========================= */
// shape names & image paths - NOW 6 SHAPES
const SHAPE_NAMES = ["circle", "square", "star", "triangle", "love", "moon"];
const SHAPE_IMAGE_PATHS = SHAPE_NAMES.map(n => `images/shape_${n}.png`);

// board images - NEW LAYOUTS
const leftBoardImg = new Image();
leftBoardImg.src = "images/board150x300.png";
const centerBoardImg = new Image();
centerBoardImg.src = "images/board300x300.png";
const rightBoardImg = new Image();
rightBoardImg.src = "images/board150x300.png";
...
/* =========================
    ======= Layout Boxes ====
    ========================= */
// NEW LAYOUT: LEFT (150x300), CENTER (300x300), RIGHT (150x300)
const leftBox = { x: 30, y: 110, width: 190, height: 380 };
const centerBox = { x: 280, y: 110, width: 400, height: 380 };
const rightBox = { x: 730, y: 110, width: 190, height: 380 };
...
function preloadAssets() {
    ...
    [leftBoardImg, centerBoardImg, rightBoardImg, coverImg, handImg, pinchImg].forEach(img => {
     promises.push(new Promise((resolve) => {
...
function generateGridPositions(box, itemSize, count) {
    ...
    // For LEFT and RIGHT boards (vertical arrangement)
    if (count === 3 && (box.width < box.height)) {
    ...
    // For CENTER board (grid arrangement)
...
function createShapesAndTargets() {
    const size = 100; // Increased by 15% (75 * 1.15 ≈ 86)
    
    // Shuffle shape names to randomize which shapes go left/right
    const shuffledNames = [...SHAPE_NAMES].sort(() => Math.random() - 0.5);
    
    // First 3 shapes go to LEFT board (vertical)
    const leftShapeNames = shuffledNames.slice(0, 3);
    const leftPositions = generateGridPositions(leftBox, size, 3);
    
    // Last 3 shapes go to RIGHT board (vertical)
    const rightShapeNames = shuffledNames.slice(3, 6);
    const rightPositions = generateGridPositions(rightBox, size, 3);
    ...
    // All 6 shapes start in CENTER board (randomly positioned)
    const centerPositions = generateGridPositions(centerBox, size, 6);
...
function drawScene() {
    ...
    // boards - NEW LAYOUT (LEFT, CENTER, RIGHT)
